...
import { syncStudio, syncClient, syncClientStats, syncStudioOwner } from '../controllers/internal.controller';
import { internalAuth } from '../middleware/internal.middleware';

const router = Router();

router.post('/studios/sync', internalAuth, syncStudio);
router.post('/studios/owners/sync', internalAuth, syncStudioOwner);
